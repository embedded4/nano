    ...
    while(true)
    {
        ...
        if(posX < 90 && posY > 400) // clear
        {
            ...
            if(confirm_clear < 0) // confirm in 10 frames before clearing
            {
                ...
                sprintf (buffer, "/var/www/image%d.png",image_num);
                cvSaveImage(buffer ,imgDrawing); // save the frame into an image
                 //cvSaveImage("/var/www/image.png",imgDrawing,0);
...
img \96 The image
pt1 \96 First point of the line segment
pt2 \96 Second point of the line segment
Type of the line:
8 - (or omitted) 8-connected line.
4 - 4-connected line.
CV_AA - antialiased line.
shift \96 Number of fractional bits in the point coordinates
#################################################################################################################
void cvInitFont(CvFont* font, int fontFace, double hscale, double vscale, double shear=0, int thickness=1, int lineType=8)
Initializes font structure.Parameters:	font \96 Pointer to the font structure initialized by the function
fontFace \96
...
param lineType:
#################################################################################################################
void cvSmooth(const CvArr* src, CvArr* dst, int smoothtype=CV_GAUSSIAN, int param1=3, int param2=0, double param3=0, double param4=0)\B6
